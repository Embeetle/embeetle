...
from typing import *
import qt, functions, functools, threading, pathlib
...
class LibstoragepathItem(_da_.File):
    class Status(_da_.File.Status):
        __slots__ = ()

        def __init__(self, item: LibstoragepathItem) -> None:
            """"""
        ...
        def sync_state(
            ...
            def sync_self(*args):
                ...
                if abspath.startswith(homedir):
                    shown_path = abspath.replace(homedir, "~", 1)
                lbltext = (
                    "location:".ljust(15, "^")
                    + f'<a href="foobar">'
                    + shown_path
    ...
    def __init__(
        ...
        """Create a LibstoragepathItem()-instance for the dashboard, to
        represent the project path of a specific library.
        ...
        super().__init__(
            ...
            rootdir=rootdir,
            parent=parent,
            name=libobj.get_local_abspath().lower(),
            state=LibstoragepathItem.Status(item=self),
        )
