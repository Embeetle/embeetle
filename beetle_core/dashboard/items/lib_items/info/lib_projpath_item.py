...
from typing import *
import qt, purefunctions, functions, functools, threading
...
class LibprojpathItem(_da_.File):
    class Status(_da_.File.Status):
        __slots__ = ()

        def __init__(self, item: LibprojpathItem) -> None:
            """"""
        ...
        def sync_state(
            ...
            def sync_self(*args) -> None:
                ...
                projpath = libobj.get_proj_relpath()
                if projpath is None:
                    purefunctions.printc(
                        f"\nERROR: LibprojpathItem()-instance detected this to\n"
                        f"be None: {q}libobj.get_proj_relpath(){q} for libobj:\n"
                        f"{q}{libobj.get_name()}{q}\n",
                        color="error",
                    )
                    projpath = "unspecified - error"
                projpath = projpath.replace("<", "&#60;").replace(">", "&#62;")
                lbltext = (
                    "location:".ljust(15, "^")
                    + f'<a href="foobar">'
                    + projpath
    ...
    def __init__(
        ...
        """Create a LibprojpathItem()-instance for the dashboard, to represent
        the project path of a specific library.
        ...
        super().__init__(
            ...
            rootdir=rootdir,
            parent=parent,
            name=libobj.get_proj_relpath().lower(),
            state=LibprojpathItem.Status(item=self),
        )
