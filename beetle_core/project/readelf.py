...
def _format_symbol_name(s):
    return _CONTROL_CHAR_RE.sub(
        lambda match: "^" + chr(0x40 + ord(match[0])), s
    )
...
class ReadElf(object):
    ...
    def display_file_header(self):
        ...
        self._emit(
            "  Start of program headers:          %s" % header["e_phoff"]
        )
        self._emitline(" (bytes into file)")
        self._emit(
            "  Start of section headers:          %s" % header["e_shoff"]
        )
        self._emitline(" (bytes into file)")
        self._emitline(
            "  Flags:                             %s%s"
    ...
    def decode_flags(self, flags):
        ...
        if self.elffile["e_machine"] == "EM_ARM":
            ...
            if (
                flags & E_FLAGS.EF_RISCV_FLOAT_ABI
            ) == E_FLAGS.EF_RISCV_FLOAT_ABI_QUAD:
    ...
    def display_program_headers(self, show_heading=True):
        ...
        if show_heading:
            ...
            self._emitline(
                "Entry point is %s" % self._format_hex(elfheader["e_entry"])
            )
                ...
                self._emitline(
                    ...
                    % (
                        ...
                        self._format_hex(
                            section["sh_offset"],
                            fieldsize=(
                                16 if section["sh_offset"] > 0xFFFFFFFF else 8
                            ),
                            lead0x=False,
                        ),
                    )
    ...
    def get_memsection_usage(self, name: str) -> Union[int, str]:
        """
        @Kristof
        Get the usage (in nr of bytes) of the given memory section.
        :return:    int -> the actual usage in nr of bytes
                    str -> reason for failure
        """
        section_name_list = []
        for section in self.elffile.iter_sections():
            section_name_list.append(section.name)
            if section.name == name:
                size_bytes = section["sh_size"]
                size_hex = self._format_hex(
                    size_bytes, fieldsize=6, lead0x=True
                )
                assert isinstance(size_bytes, int)
                return size_bytes
        return f"INFO: section '{name}' not in elf file"
        ...
        for section_index, section in symbol_tables:
            ...
            for nsym, symbol in enumerate(section.iter_symbols()):
                ...
                if (
                    ...
                    symbol_name = self.elffile.get_section(
                        symbol["st_shndx"]
                    ).name
        ...
        for ehabi_info in self.elffile.get_ehabi_infos():
            ...
            for i in range(ehabi_info.num_entry()):
                ...
                if isinstance(entry, GenericEHABIEntry):
                    ...
                    self._emitline(
                        "    Compact model index: %d" % entry.personality
                    )
            ...
            if isinstance(section, GNUVerSymSection):
                ...
                for verdef, verdaux_iter in section.iter_versions():
                    ...
                    self._emitline(
                        ...
                        % (
                            self._format_hex(
                                offset, fieldsize=6, alternate=True
                            ),
                            verdef["vd_version"],
                            flags,
                            verdef["vd_ndx"],
    ...
    def display_hex_dump(self, section_spec):
        """Display a hex dump of a section.

        section_spec is either a section number or a name.
        """
    ...
    def display_string_dump(self, section_spec):
        """Display a strings dump of a section.
        ...
        while dataptr < len(data):
            ...
            found = True
            self._emitline(
                "  [%6x]  %s" % (dataptr, bytes2str(data[dataptr:endptr]))
            )
    ...
    def _print_version_section_header(
        ...
        verneed or verdef) with some options to accomodate readelf little
        differences between each header (e.g. indentation and 0x prefixing)."""
    ...
    def _init_versioninfo(self):
        """Search and initialize informations about version related sections and
        the kind of versioning used (GNU or Solaris)."""
    ...
    def _symbol_version(self, nsym):
        """Return a dict containing information on the or None if no version
    ...
    def _get_symbol_shndx(self, symbol, symbol_index, symtab_index):
        """Get the index into the section header table for the "symbol" at
        "symbol_index" located in the symbol table with section index
        "symtab_index"."""
        ...
        return self._shndx_sections[symtab_index].get_section_index(
            symbol_index
        )

    def _note_relocs_for_section(self, section):
        """If there are relocation sections pointing to the givne section, emit
        a note about it."""
            ...
            if cu["version"] >= 5:
                if cu.header.get("unit_type", ""):
                    ...
                    self._emitline(
                        ...
                        % (
                            self._format_hex(
                                cu["debug_abbrev_offset"], alternate=True
                            )
                        )
                    )
                    ...
                    if unit_type in ("DW_UT_skeleton", "DW_UT_split_compile"):
                        ...
                        self._emitline(
                            "   Signature:     0x%x" % cu["type_signature"]
                        )
                        self._emitline(
                            "   Type Offset:   0x%x" % cu["type_offset"]
                        )
            else:
                self._emitline(
                    "   Abbrev Offset: %s"
            ...
            for die in cu.iter_DIEs():
                ...
                for attr in die.attributes.values():
                    ...
                    if (
                        ...
                        and not "DW_AT_frame_base"
                        in current_function.attributes
                    ):
    ...
    def _dump_debug_line_programs(self):
        """Dump the (decoded) line programs from .debug_line The programs are
        dumped in the order of the CUs they belong to."""
        if not self._dwarfinfo.has_debug_info:
            return
        self._emitline(
            ...
            for entry in lineprogram.get_entries():
                ...
                if state is None:
                    ...
                    if entry.command == DW_LNS_set_file:
                        file_entry = lineprogram["file_entry"][
                            entry.args[0] - 1
                        ]
                    ...
                    self._emitline(
                        ...
                        % (
                            ...
                            (
                                "x"
                                if state.is_stmt and not state.end_sequence
                                else ""
                            ),
                        )
                    )
            ...
            if isinstance(entry, CIE):
                self._emitline(
                    ...
                    % (
                        entry.offset,
                        self._format_hex(
                            entry["length"], fullhex=True, lead0x=False
                        ),
                        self._format_hex(
                            entry["CIE_id"], fieldsize=8, lead0x=False
                        ),
                    )
                )
    ...
    def _dump_debug_namelut(self, what):
        ...
        for cu_hdr, (cu_ofs, items) in izip(
            cu_headers,
            itertools.groupby(namelut.items(), key=lambda x: x[1].cu_ofs),
        ):
            ...
            self._emitline(
                "  Version:                             %d" % cu_hdr.version
            )
            self._emitline(
                "  Offset into .debug_info section:     0x%x"
                % cu_hdr.debug_info_offset
            )
            self._emitline(
                "  Size of area in .debug_info section: %d"
                % cu_hdr.debug_info_length
            )
            self._emitline()
    ...
    def _dump_debug_aranges(self):
        ...
        self._emitline(
            "Contents of the %s section:"
            % self._dwarfinfo.debug_aranges_sec.name
        )
        self._emitline()
        prev_offset = None
        for entry in unordered_entries:
            if prev_offset != entry.info_offset:
                ...
                self._emitline(
                    "  Length:                   %d" % (entry.unit_length)
                )
                self._emitline(
                    "  Version:                  %d" % (entry.version)
                )
                self._emitline(
                    "  Offset into .debug_info:  0x%x" % (entry.info_offset)
                )
                self._emitline(
            ...
            if entry.begin_addr != 0 or entry.length != 0:
                self._emitline(
                    ...
                    % (
                        self._format_hex(
                            entry.begin_addr, fullhex=True, lead0x=False
                        ),
                        self._format_hex(
                            entry.length, fullhex=True, lead0x=False
                        ),
    ...
    def _dump_frames_interp_info(self, section, cfi_entries):
        ...
        for entry in cfi_entries:
            if isinstance(entry, CIE):
                ...
                if len(entry.get_decoded().table) == len(
                    entry.cie.get_decoded().table
                ):
                    continue
            ...
            self._emit("   LOC")
            self._emit(
                "  " if entry.structs.address_size == 4 else "          "
            )
            self._emit(" CFA      ")
    ...
    def _dump_debug_locsection(self, di, loc_lists_sec):
        ...
        section_name = (
            di.debug_loclists_sec if ver5 else di.debug_loc_sec
        ).name
            ...
            for die in cu.iter_DIEs():
                for key in die.attributes:
                    ...
                    if LocationParser.attribute_has_location(
                        attr, cu["version"]
                    ) and LocationParser._attribute_has_loc_list(
                        attr, cu["version"]
                    ):
        ...
        line_template = "    %%08x %%0%dx %%0%dx %%s%%s" % (
            addr_width,
            addr_width,
        )
        ...
        for entry in loc_list:
            if isinstance(entry, LocationViewPair):
                ...
                if cu_map is None:
                    ...
                    if not cu:
                        raise ValueError(
                            "Location list can't be tracked to a CU"
                        )
                ...
                if isinstance(entry, LocationEntry):
                    ...
                    expr = describe_DWARF_expr(
                        entry.loc_expr, cu.structs, cu.cu_offset
                    )
                    if has_views:
    ...
    def _dump_debug_rnglists_CU_header(self, cu):
        ...
        self._emitline(
            "  Length:          %s"
            % self._format_hex(cu.unit_length, alternate=True)
        )
        ...
        if cu.offsets and len(cu.offsets):
            self._emitline(
                "  Offsets starting at 0x%x:" % cu.offset_table_offset
            )
    ...
    def _dump_debug_rangesection(self, di, range_lists_sec):
        ...
        section_name = (
            di.debug_rnglists_sec if ver5 else di.debug_ranges_sec
        ).name
        addr_size = di.config.default_address_size  # In bytes, 4 or 8
        addr_width = addr_size * 2  # In hex digits, 8 or 16
    ...
    def _dump_rangelist(
            ...
            if isinstance(entry, RangeEntry):
                ...
                if ver5:
                    if not raw_v5_rangelist:
                        raw_v5_rangelist = (
                            range_lists_sec.get_range_list_at_offset_ex(
                ...
                if (
                    raw_v5_entry
                    and raw_v5_entry.entry_type == "DW_RLE_base_addressx"
                ):
                    self._emitline(
                        base_template_indexed
                        % (
        ...
        self._emitline(
            "    %08x <End of list>"
            % (
                last.entry_offset + last.entry_length
                if ver5
                else first.entry_offset
            )
        )
    ...
    def _display_arch_specific_riscv(self):
        """Display the RISC-V architecture-specific info contained in the
        file."""
...
def main(stream=None):
    ...
    argparser.add_argument(
        "file", nargs="?", default=None, help="ELF file to parse"
    )
    argparser.add_argument(
