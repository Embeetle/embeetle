...
Parses string output from gdb with the `--interpreter=mi2` flag into structured
objects.
...
def _parse_dict(stream: StringStream):
    """Parse dictionary, with optional starting character '{' return (tuple):

    Number of characters parsed from to_parse Parsed dictionary
    """
...
def _parse_key_val(stream: StringStream):
    """Parse key, value combination return (tuple):

    Parsed key (string) Parsed value (either a string, array, or dict)
    """
