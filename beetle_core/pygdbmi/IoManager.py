...
"""This module defines the `IoManager` class which manages I/O for file objects
connected to an existing gdb process or pty."""
import io
import select
import time
...
class IoManager(qt.QObject):
    ...
    def __init__(
        ...
        """Manage I/O for file objects created before calling this class This
        can be useful if the gdb process is managed elsewhere, or if a pty is
        used."""
        ...
        self._incomplete_output: Dict[str, Any] = {
            "stdout": None,
            "stderr": None,
        }
    ...
    def __read_loop(self):
        while self.__running:
            try:
                time.sleep(0.01)
                responses = self.get_gdb_response()
                if responses is not None and len(responses) > 0:
                    self.data_received_signal.emit(responses)
                else:
                    time.sleep(0.1)
    ...
    def stop(self):
        """Stop all threaded operation."""
        self.__running = False

    def get_gdb_response(self):
        """Get response from GDB, and block while doing so. If GDB does not have
        any response ready to be read by timeout_sec, an exception is raised.
    ...
    def _get_responses_windows(self, timeout_sec):
        """Get responses on windows.

        Assume no support for select and use a while loop.
        """
    ...
    def _get_responses_list(
        ...
        response_list = list(
            filter(
                lambda x: x, _new_output.decode(errors="replace").split("\n")
            )
        )  # remove blank lines
    ...
    def write(self, mi_cmd_to_write: Union[str, List[str]]):
        """Write to gdb process. Block while parsing responses from gdb for a
        maximum of timeout_sec.
        ...
        if USING_WINDOWS:
            ...
            _, outputready, _ = select.select(
                [], self.write_list, [], timeout_sec
            )
...
def _buffer_incomplete_responses(
    ...
    """It is possible for some of gdb's output to be read before it completely
    finished its response. In that case, a partial mi response was read, which
    cannot be parsed into structured data. We want to ALWAYS parse complete mi
    records. To do this, we store a buffer of gdb's output if the output did not
    end in a newline.
