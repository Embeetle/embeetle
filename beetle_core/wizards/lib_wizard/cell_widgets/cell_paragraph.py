...
class ParagraphLabel(_cell_widget_.CellLabel):
    """"""
    ...
    def __init__(
        ...
        super().__init__(
            ...
            col=col,
            libobj=libobj,
            text=libobj.get_mod_paragraph(),
            gridlyt=gridlyt,
            parent=parent,
            minwidth=minwidth,
            maxwidth=maxwidth,
        )
        # self.destroyed.connect(lambda: print(f"[{libname}] destroyed col 6"))
        return

    def accept_new_libobj(self, new_libobj: _libobj_.LibObj) -> None:
        """Accept new libobj for oneself.

        Repoint the reference and make modi- fications if needed.
        """
        super().accept_new_libobj(new_libobj)
        self.setText(new_libobj.get_mod_paragraph())
        self.update()
        return
    ...
    def self_destruct(
        ...
        if not death_already_checked:
            if self.dead:
                raise RuntimeError(f"Trying to kill ParagraphLabel() twice!")
            self.dead = True
