...
class CustomInterpreter(code.InteractiveInterpreter):
    ...
    dict_re_references = dict(
        ...
        toggle_main_window_side=(
            r"^toggle_main_window_side$",
            r"toggle_main_window_side()",
        ),
        append=(r"^append\(", r"cmain.append_to_lines("),
        append_all=(
            r"^append_all\((.*)\)",
            r"cmain.append_to_lines(\g<1>, 1,cmain.lines())",
        ),
        prepend=(r"^prepend\(", r"cmain.prepend_to_lines("),
        prepend_all=(
            r"^prepend_all\((.*)\)",
            r"cmain.prepend_to_lines(\g<1>, 1,cmain.lines())",
        ),
        close_tab=(r"^close_tab\(\)|^close_tab", r"main.close_tab()"),
        undo_all=(
            r"^undo_all\(\)$|^undo_all$",
            r"main.currentWidget().undo_all()",
        ),
        redo_all=(
            r"^redo_all\(\)$|^redo_all$",
            r"main.currentWidget().redo_all()",
        ),
        indent_with_tabs=(
            r"^indent_with_tabs\(\)$|^indent_with_tabs$",
            r"main.currentWidget().indent_with_tabs(True)",
        ),
        indent_with_spaces=(
            r"^indent_with_spaces\(\)$|^indent_with_spaces$",
            r"main.currentWidget().indent_with_tabs()",
        ),
        tabs_to_spaces=(
            r"^tabs_to_spaces\(\)$|^tabs_to_spaces$",
            r"main.currentWidget().tabs_to_spaces()",
        ),
        show_edge_marker=(
            r"^show_edge_marker\(\)$|^show_edge_marker$",
            r"main.currentWidget().show_edge_marker()",
        ),
        hide_edge_marker=(
            r"^hide_edge_marker\(\)$|^hide_edge_marker$",
            r"main.currentWidget().hide_edge_marker()",
        ),
        reload_file=(
            r"^reload_file|^reload_file\(\)",
            r"main.currentWidget().reload_file()",
        ),
        show_node_tree=(
            r"^show_node_tree|^show_node_tree\(\)",
            r"show_node_tree(main.currentWidget())",
        ),
        for_each_line=(
            r"(?<!\.)(?<!{:s})for_each_line\(".format(re_escape_sequence),
            r"cmain.for_each_line(",
        ),
        remove_empty_lines=(
            r"(?<!\.)(?<!{:s})remove_empty_lines\(".format(re_escape_sequence),
            r"cmain.remove_empty_lines(",
        ),
    )
    ...
    def __init__(self, get_references_func, print_func):
        """Initialize the interactive interpreter."""
        # Get the default references and add them to the initial_references list
        initial_references, ac_list_prim, ac_list_sec = get_references_func()
        initial_references.update(self.get_default_references())
        # The interpreter object takes references as the first argument
        ...
        # Save the function for getting references
        self.get_references_func = get_references_func

    def eval_command(self, command, display_action=True):
        """This EVAL function executes the command by the interactive
        interpreter.
        ...
        try:
            ...
            filtered_command = self.replace_references(
                self.dict_re_references, command
            )
    ...
    def replace_references(self, references, command):
        """Replace references with actual forms module methods and
        attributes."""
        ...
        if os_checker.is_os("windows"):
            ...
            if show_console == True:
                ...
                if output_to_repl == True:
                    ...
                    if '"' in command:
                        sub_call = (
                            "subprocess.Popen('cmd.exe /c {:s}', {:s})".format(
                                command, options
                            )
                        )
                    else:
                        sub_call = 'subprocess.Popen(["cmd.exe", "/c", "{:s}"], {:s})'.format(
                            command, options
                        )
                    process_commands = [
            ...
            if show_console == True:
                ...
                if output_to_repl == True:
                    ...
                    sub_call = 'subprocess.Popen("{:s}", {:s})'.format(
                        command, options
                    )
                    process_commands = [
                        "result = " + sub_call,
    ...
    def create_terminal(self, dir=None):
        """Spawn a new "terminal/command line" window asynchronously (currently
        works only on win32)"""
