...
class LineList(list):
    ...
    def __setitem__(self, key, value):
        """Overridden list method that sets the specified line(item) to a
        value."""
    ...
    def _setitem(self, key, value):
        """Set the item at position-key, without updating the scintilla
        document."""
    ...
    def append(self, value, update_parent=True):
        ...
        if isinstance(update_parent, bool) == False:
            raise Exception(
                "'update_parent' parameter must be of type boolean!"
            )
    ...
    def update_text_to_list(self, update_text):
        ...
        # Empty the list
        self._clear()
        # Get the line ending symbol
        separator = functions.get_separator(update_text)
        # Set the new content
        self.extend(re.split(separator, update_text), update_parent=False)
