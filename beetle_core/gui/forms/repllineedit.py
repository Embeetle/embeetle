...
class ReplLineEdit(qt.QLineEdit):
    ...
    def __init__(self, parent, interpreter_references=None):
        ...
        self.interpreter = components.interpreter.CustomInterpreter(
            interpreter_references, parent.display.display_message_with_type
        )
    ...
    def _get_path_list(self, path_string):
        """Return a list of all directories and files if the path string is
        valid."""
        path_list = []
    ...
    def _repl_eval(self, external_string=None, display_action=True):
        ...
        if display_action == True:
            ...
            for i, command in enumerate(split_command):
                if i != 0:
                    self.parent.display.display_message_with_type(
                        "... " + command
                    )
                else:
                    self.parent.display.display_message_with_type(
            ...
            if current_rm_index is not None:
                # Revert the focus of the gui.forms.tabwidget.TabWidget that hold the REPL messages tab to
        ...
        eval_return = self.interpreter.eval_command(
    ...
    def _create_ac_cycler(self, autocompletion_list):
        """Create a autocompletion cycler to cycle through all of the
        autocompletions."""
    ...
    def _input_buffer_cycle(self, direction=0):
        ...
        if direction == 1:
            ...
            self.setText(
                self._input_buffer["list"][self._input_buffer["count"]]
            )
        else:
    ...
    def _get_valid_sequence_list(self, string):
        """Get the last one or two valid sequence, which will be compared to
        autocompletion list E.G.

        1: "print(main.se" returns >> first = "se", second = "main"
    ...
    def event(self, event):
        """Rereferenced/overloaded main QWidget event, that is executed before
        all other events of the widget."""
    ...
    def _keypress_decorator(func):
        """A decorator for the QScintila KeypressEvent, to catch which key was
        pressed."""
    ...
    def interpreter_update_references(
        ...
        """Update the references that can be accessed by the interactive
        interpreter."""
    ...
    def interpreter_add_references(self, new_references):
        ...
        self._list_first_level_completions = list(
            self._list_first_level_completions
        )
    ...
    def interpreter_reset_references(
        ...
        """Clear all of the interpreter references and update them with the new
        ones."""
    ...
    def interpreter_update_windows(self, main, upper, lower):
        """Update the Main, Upper and Lower window references of the
        interpreter."""
    ...
    def skip_next_repl_focus(self):
        """Function that sets the flag that makes the next REPL evaluation skip
        setting focus back to the ReplLineEdit."""
        # Set the skip focus flag
        self._repl_focus_flag = True

    def get_repl_references(self):
        """Create and return a dictionary that holds all the REPL references
        that will be used in the interpreter module."""
        return dict(
            repl=self,
    ...
    def repeat_last_repl_eval(self):
        ...
        if len(self._input_buffer["list"]) > 0:
            ...
            self.parent.display.write_to_statusbar(
                "No commands in REPL input buffer!", 1000
            )

    def external_eval_request(self, eval_string, calling_widget):
        """An external evaluation request from the ReplHelper or another
        widget."""
