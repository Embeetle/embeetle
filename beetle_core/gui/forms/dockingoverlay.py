...
class DockingOverlay:
    ...
    class CentralDockLabel(BaseDockLabel):
        def dropEvent(self, event):
            ...
            #            main_form._print_all_boxes_and_windows()
    ...
    def __init__(self, parent):
        if not isinstance(parent, gui.forms.mainwindow.MainWindow):
            raise ValueError(
                "Docking overlay needs a MainWindow parent, "
                + "not {}!".format(parent.__class__)
            )
        self.parent = parent

    def show_on_parent(self, docking_widgets):
        for items in docking_widgets:
            widget = items
            widget.close_button_hide()
