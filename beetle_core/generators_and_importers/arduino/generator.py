...
def generate_projects() -> None:
    """Generating the Arduino projects happens in a single step (unlike the
    STMicro projects).

    That's because the Arduino projects in 'C:/sample_proj_resources' are fully
    completed Arduino projects (even though they're just simply one '.ino'
    sketch file per project). The only needed step then is to import them.
    """
    ...
    project_list = purefunctions.load_json_file_with_comments(
        f"{data.sample_proj_resources}/arduino/projects/project_list.json5"
    )
    for project_name in project_list.keys():
        # $ Extract paths from 'project_list.json5'
        relpath = project_list[project_name]
        src_abspath = f"{data.sample_proj_resources}/arduino/projects/{relpath}"
        dst_abspath = f"{data.generated_projects_directory}/arduino/{relpath}"
        # $ Read 'hardware.json5'
        ...
        # Check if there's a .ino file in there. If yes, import it as usual for Arduino projects. If
        # not, import the whole folder in the general way.
        arduino_way = os.path.isfile(f"{src_abspath}/{project_name}.ino")
        functions.importer_printfunc(
            f"arduino import = ", color="yellow", end=""
        )
        functions.importer_printfunc(arduino_way)
        _helper_.import_project(
            manufacturer="arduino" if arduino_way else "other",
...
def zip_projects() -> None:
    ...
    for project_name in project_list.keys():
        # $ Extract paths from 'project_list.json5'
        relpath = project_list[project_name]
        proj_abspath = f"{data.generated_projects_directory}/arduino/{relpath}"
        zip_abspath = f"{data.zipped_projects_directory}/arduino/{relpath}.7z"
