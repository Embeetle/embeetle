...
def generate_projects() -> None:
    ...
    project_list = purefunctions.load_json_file_with_comments(
        f"{data.sample_proj_resources}/giga/projects/project_list.json5"
    )
    for project_name in project_list.keys():
        # $ Extract paths from 'project_list.json5'
        relpath = project_list[project_name]
        src_abspath = f"{data.sample_proj_resources}/giga/projects/{relpath}"
        dst_abspath = f"{data.generated_projects_directory}/giga/{relpath}"
        # $ Read 'hardware.json5'
...
def zip_projects() -> None:
    ...
    for project_name in project_list.keys():
        # $ Extract paths from 'project_list.json5'
        relpath = project_list[project_name]
        proj_abspath = f"{data.generated_projects_directory}/giga/{relpath}"
        zip_abspath = f"{data.zipped_projects_directory}/giga/{relpath}.7z"
