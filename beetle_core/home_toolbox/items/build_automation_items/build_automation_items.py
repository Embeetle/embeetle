...
class BuildAutomationRootItem(_tm_.Root):
    class Status(_tm_.Folder.Status):
        __slots__ = ()

        def __init__(self, item: BuildAutomationRootItem) -> None:
            """ """
        ...
        def sync_state(
            ...
            def sync_self(*args) -> None:
                self.lblTxt = (
                    ...
                    else "Build Automation "
                )
                self.closedIconpath = f"icons/tools/build_automation.png"
                self.openIconpath = f"icons/tools/build_automation.png"
    ...
    def __init__(self) -> None:
        """ """
        super().__init__(
            name="BuildAutomationRootItem",
            state=BuildAutomationRootItem.Status(item=self),
        )
...
class BuildAutomationItem(_tm_.Folder):
    class Status(_tm_.Folder.Status):
        __slots__ = ()
        ...
        def sync_state(
            ...
            always!
            """
            build_automation_item: BuildAutomationItem = cast(
                BuildAutomationItem,
                self.get_item(),
            )
            toolman_obj: _tool_obj_.ToolmanObj = cast(
                _tool_obj_.ToolmanObj,
                build_automation_item.get_toolObj(),
            )
            ...
            def sync_self_b(*args) -> None:
                ...
                # GNU Make generally has no suffix to show. So the
                # bitness should be shown instead!
    ...
    def __init__(
        ...
        rootdir: BuildAutomationRootItem,
    ) -> None:
        """ """
        super().__init__(
            rootdir=rootdir,
            parent=rootdir,
            name="BuildAutomationItem",
            state=BuildAutomationItem.Status(item=self),
            toolObj=toolmanObj,
        )
        assert isinstance(toolmanObj, _tool_obj_.ToolmanObj)
        assert isinstance(rootdir, BuildAutomationRootItem)
        self.get_state().sync_state(
            refreshlock=False,
            callback=None,
            callbackArg=None,
