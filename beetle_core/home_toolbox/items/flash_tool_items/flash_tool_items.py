...
class FlashToolRootItem(_tm_.Root):
    class Status(_tm_.Folder.Status):
        __slots__ = ()

        def __init__(self, item: FlashToolRootItem) -> None:
            """"""
        ...
        def sync_state(
            ...
            def sync_self(*args) -> None:
                self.lblTxt = (
                    "*Flashtool" if self.has_asterisk() else "Flashtool "
                )
                self.closedIconpath = f"icons/tools/flashtool.png"
                self.openIconpath = f"icons/tools/flashtool.png"
    ...
    def __init__(self) -> None:
        """"""
        super().__init__(
            name="FlashToolRootItem",
            state=FlashToolRootItem.Status(item=self),
        )
...
class FlashToolItem(_tm_.Folder):
    class Status(_tm_.Folder.Status):
        __slots__ = ()

        def __init__(self, item: FlashToolItem) -> None:
            """"""
            super().__init__(item=item)
            self.closedIconpath = None
            self.openIconpath = None
        ...
        def sync_state(
            ...
            always!
            """
            flashtool_item: FlashToolItem = cast(
                FlashToolItem,
                self.get_item(),
            )
            toolman_obj: _tool_obj_.ToolmanObj = cast(
                _tool_obj_.ToolmanObj,
                flashtool_item.get_toolObj(),
            )
            ...
            def sync_self_b(*args) -> None:
                ...
                # OpenOCD got stuck at version 0.10.0 and you should rely
                # on the dev-nr suffix to differentiate!
    ...
    def __init__(
        ...
        super().__init__(
            ...
            parent=rootdir,
            name="FlashToolItem",
            state=FlashToolItem.Status(item=self),
            toolObj=toolmanObj,
        )
        assert isinstance(toolmanObj, _tool_obj_.ToolmanObj)
        assert isinstance(rootdir, FlashToolRootItem)
        self.get_state().sync_state(
            refreshlock=False,
            callback=None,
            callbackArg=None,
